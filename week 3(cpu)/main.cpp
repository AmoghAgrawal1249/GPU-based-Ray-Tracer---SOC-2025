...
color ray_color(const ray& r, const hittable& world) {
    ...
    if (world.hit(r, interval(0, infinity), rec)) {
        return 0.5f * (rec.surface_normal + color(1.0f, 1.0f, 1.0f));
    }
...
int main() {
    ...
    point3 pixel00_location = viewport_upper_left + 0.5f * (pixel_delta_horizontal + pixel_delta_vertical);
    ...
    for (int row = 0; row < image_height; row++) {
        ...
        for (int column = 0; column < image_width; column++) {
            point3 pixel_center = pixel00_location + (float)column * pixel_delta_horizontal + (float)row * pixel_delta_vertical;
